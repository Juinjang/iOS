...
final class ToSItemTableViewCell: UITableViewCell {
    ...
    @objc func openContentButtonPressed(_ sender: UIButton) {
        ...
        let tos2VC = ToS2ViewController()
        let tos3VC = ToS3ViewController()
        
        if let parentVC = parentViewController {
            if tag == 0 {
                parentVC.navigationController?.pushViewController(tos1VC, animated: true)
            } else if tag == 1 {
                parentVC.navigationController?.pushViewController(tos2VC, animated: true)
            } else if tag == 2 {
                parentVC.navigationController?.pushViewController(tos3VC, animated: true)
            }
        }
    }
