...
final class ToSViewController: BaseViewController, PopUpDelegate {
    func didTapCancelButton() {
    ...
    @objc private func agreeLabelTapped(sender: UITapGestureRecognizer) {
        // 동의하기 버튼을 클릭한 것처럼 처리
...
//            $0.bottom.equalTo(view.safeAreaLayoutGuide.snp.bottom).offset(-33)
    ...
    @objc private func handleCheckButtonChecked() {
        areAllTermsChecked()
        isEssentialTermsChecked()
    }
    
    @objc private func handleCheckButtonUnchecked() {
        areNotAllTermsChecked()
    }
    
    private func areAllTermsChecked() {
    ...
    private func indexPathForTag(_ tag: Int) -> IndexPath? {
